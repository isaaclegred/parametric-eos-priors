...
for  n, eos in enumerate (eos_list):
    ...
    P2nucs.append(np.log10((find_pressure_of_density(2 * nuc_rho_si, eos))/ccgs**2*10)) # SI -> cgs is x 10
    P6nucs.append(np.log10((find_pressure_of_density(6 * nuc_rho_si, eos))/ccgs**2*10))
...
# Pressure(2 rho_nuc)
plt.figure()
plt.hist(P6nucs, bins=30)
plt.xlabel(r"$\log_{10}(P(6\rho_{nuc})/c^2$)")
plt.ylabel("frequency")
plt.savefig("better_prior_P2nuc.png")
plt.show()
